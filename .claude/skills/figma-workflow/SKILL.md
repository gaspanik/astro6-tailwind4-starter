...
name: figma-workflow
description: >-
  Figma URL を受け取り、実装 → レビュー → 最適化 の3フェーズをサブエージェントで逐次実行するワークフロースキル（Astro向け）
...
Figma URL を受け取り、**実装 → レビュー → 最適化** の3フェーズをサブエージェントで逐次実行するワークフロースキルです。
...
- 必ず **Phase 1 → Phase 2 → Phase 3** の順に実行する（並列不可）
